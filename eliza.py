...
REGEXPS = [re.compile(p[0], re.IGNORECASE) for p in PATS]
RESPONSES = [p[1] for p in PATS]
...
def reflect(sentence):
    ...
    for i in range(0, len(words)):
        words[i] = REFLECTIONS.get(words[i].lower(), words[i])
...
def respond(sentence):
    for i in range(len(REGEXPS)):
        ...
        match = regexp.match(sentence)
        if match:
            response = random.choice(responses)
            groups = [reflect(s) for s in match.groups()]
